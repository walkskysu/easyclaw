...
- --timeout sets the command timeout in milliseconds. **Maximum enforced timeout is 20000 (20 seconds).** Any value above 20000 will be capped at 20000.
...
- All agent-browser commands are subject to a **hard 20-second execution timeout**. Long-running operations should use `--timeout 19000` (or less) and the `yieldMs` option if background execution is needed.
